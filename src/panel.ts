...
import { MeterSnapshot, formatTokens } from "./meter";
import { BloatHit, formatBytes } from "./bloat";
...
export class GovernorPanel implements vscode.WebviewViewProvider {
  ...
  setMeter(meter: MeterSnapshot): void {
    this.meter = meter;
    this.refresh();
  }

  setBloat(bloat: BloatHit[]): void {
    this.bloat = bloat;
    this.refresh();
  }
  ...
  private render(): string {
    ...
    const fileRows = files.slice(0, 8).map((f) =>
      `<tr><td>${escapeHtml(f.path)}</td><td class="num">${formatTokens(f.tokens)}</td><td class="num">${f.lines}</td></tr>`
    ).join("");
    const bloatRows = this.bloat.slice(0, 8).map((b) =>
      `<tr><td>${escapeHtml(b.path)}</td><td class="num">${formatBytes(b.bytes)}</td><td>${escapeHtml(b.reason)}</td></tr>`
    ).join("");
...
<body>
  ...
  <div class="gauge"><i style="width:${Math.min(100, Math.round(tokens / 50))}%"></i></div>
  <div class="actions">
